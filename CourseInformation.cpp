...
// Description : CoursePlanner in C++
//============================================================================
...
void HashTable::Insert(Bid bid) {
    ...
    unsigned key = hash(atoi(bid.courseId.c_str()));
...
void HashTable::PrintAll() {
    ...
    for (size_t i = 0; i < nodes.size(); ++i) {
        //iterate through nodes in the linked list in current bucket
        for (Node* current = &nodes[i]; current != nullptr; current = current->next) {
            //iterate through rest of list to compare to previous nodes
            for (Node* nextNode = current->next; nextNode != nullptr; nextNode = nextNode->next) {
                //if the current node course id is greater that the next node course id
                if (current->bid.courseId > nextNode->bid.courseId) {
                    // swap the bids
                    Bid temp = current->bid;
                    current->bid = nextNode->bid;
                    nextNode->bid = temp;
                }
            }
        }
    ...
    for (const Node& node : nodes) {
        ...
        if (node.key != UINT_MAX) {
            ...
            cout << node.bid.courseId << ", " << node.bid.courseTitle << ", " << node.bid.prerequisites << endl;
            // node is equal to next iteR
            Node* currentNode = node.next;
            // while node not equal to nullptr
            while (currentNode != nullptr) {
                // output key, bidID, title, amount and fund
                cout << currentNode->bid.courseId << ", " << currentNode->bid.courseTitle << ", " << currentNode->bid.prerequisites << endl;
 ...
 */
Bid HashTable::Search(string courseId) {
    Bid bid;
    ...
    if (node != nullptr && node->key != UINT_MAX && node->bid.courseId.compare(courseId) == 0) {
        //return node bid
        return node->bid;
    }
    ...
    while (node != nullptr) {
        ...
        if (node->key != UINT_MAX && node->bid.courseId.compare(courseId) == 0) {
            return node->bid;
        }
    ...
    while (choice != 9) {
        ...
        case 3:
            ...
            if (!bid.courseId.empty()) {
                displayBid(bid);
                cout << "Prerequisites: " << bid.prerequisites << endl << endl;
            }
            else {
                cout << "Course Id " << bidKey << " not found." << endl << endl;
            }
            break;
        }
    }
